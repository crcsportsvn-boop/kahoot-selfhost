...
export default function LobbyView({
  ...
  return (
    <div className="w-full min-h-[calc(100vh-4rem)] flex flex-col justify-between p-4 sm:p-8 text-white relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-6 bg-slate-900/70 backdrop-blur-xl border border-slate-800 p-6 rounded-3xl shadow-2xl">
        <div className="flex items-center gap-4">
          {logoUrl && (
            <div className="p-2 rounded-2xl bg-white/5 border border-slate-800 shadow-md">
              <img
                src={logoUrl}
                alt="Presentation Logo"
                className="h-14 max-w-[160px] object-contain rounded-xl"
                onError={(e) => {
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-4 gap-6 my-8 flex-1 items-start">
        ...
        <div className="lg:col-span-1 bg-slate-900/60 backdrop-blur-xl border border-slate-800 p-6 rounded-3xl flex flex-col items-center justify-center text-center shadow-2xl">
          ...
          <p className="text-xs font-semibold text-slate-300 mb-1">{t.scanCameraHelp}</p>
          <p className="text-[11px] text-slate-500 break-all font-mono">{joinUrl}</p>
        </div>
        ...
        <div className="lg:col-span-3 bg-slate-900/60 backdrop-blur-xl border border-slate-800 p-6 rounded-3xl flex flex-col min-h-[360px] shadow-2xl">
          ...
          <div className="flex items-center justify-between border-b border-slate-800/80 pb-4 mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">{t.playersInRoom}</h2>
                <p className="text-xs text-slate-400">{t.playersAutoUpdate}</p>
              </div>
            </div>
          ...
          {players.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center py-12">
              ...
              <p className="text-lg font-bold text-slate-300">{t.waitingPlayers}</p>
              <p className="text-xs text-slate-500 max-w-sm mt-1">
                {t.openToJoin} <span className="text-purple-400 font-mono font-semibold">/play/{pin}</span>
              </p>
            </div>
          ) : (
              ...
              {players.map((p) => (
                ...
                >
                  ...
                  <div className="overflow-hidden">
                    ...
                    <span className="text-[10px] text-emerald-400 font-semibold">{t.readyBadge}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-900/80 backdrop-blur-xl border border-slate-800 p-4 rounded-2xl shadow-2xl">
        <div className="text-xs text-slate-400 flex items-center gap-2">
          <Volume2 className="w-4 h-4 text-indigo-400 shrink-0" />
          <span>{t.lobbyMusicNotice}</span>
        </div>

        <button
          onClick={onStartGame}
          disabled={players.length === 0 || isStarting}
          className="w-full sm:w-auto inline-flex items-center justify-center gap-3 px-8 py-3.5 rounded-xl font-black text-lg bg-gradient-to-r from-emerald-500 via-teal-500 to-green-600 hover:from-emerald-400 hover:to-green-500 active:scale-95 text-white shadow-xl shadow-emerald-700/30 disabled:opacity-40 disabled:cursor-not-allowed transition cursor-pointer"
        >
          <Play className="w-5 h-5 fill-current" />
          <span>{isStarting ? t.startingBtn : t.startGameBtn}</span>
        </button>
      </div>
    </div>
  );
